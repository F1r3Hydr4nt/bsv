...
class TxBuilder extends Struct {
  ...
  /**
     ...
     */
  inputFromScript (txHashBuf, txOutNum, txOut, script, nSequence = 0xffffffff) {
    if (
  ...
  estimateSize () {
    ...
    const scriptContextSizeMinusVarintScriptCode = 4 + 32 + 32 + 36 + 8 + 4 + 32 + 4 + 4
    ...
    this.tx.txIns.forEach((txIn) => {
      ...
      sigOperations.forEach((obj) => {
        ...
        if (type === 'sig') {
          ...
          size += pubKeySize
        } else if (obj.type === 'scriptContext') {
          size += scriptContextSizeMinusVarintScriptCode + txOut.script.toBuffer().length + txOut.scriptVi.toBuffer().length
        } else {
  ...
  build (opts = { useAllInputs: false }) {
    ...
    if (this.changeAmountBn.geq(minFeeAmountBn)) {
      ...
      if (this.changeAmountBn.lt(this.dust)) {
        ...
        // Remove the change amount since it is pointless as it is zero.
        this.tx.txOuts.pop()
        this.tx.txOutsVi = VarInt.fromNumber(this.tx.txOutsVi.toNumber() - 1)
    ...
    for (const nIn in this.tx.txIns) {
      ...
      for (const obj of arr) {
        ...
        if (type === 'sig') {
          ...
          console.error(`cannot perform operation of type ${type}`)
          continue
        }
      }
    }
